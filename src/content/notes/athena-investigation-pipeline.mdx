...
Read that and the decision becomes simple. Pulling and summarizing a single source is easy enough that a small local model does it for nearly free; correlation and the final verdict are where judgment lives, so those earn the bigger model. The point isn't "local good, cloud bad." It's that you can only make the trade deliberately once you've measured it.
