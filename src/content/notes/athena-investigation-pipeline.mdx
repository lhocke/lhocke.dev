...
```text
before:  Customer Jane Okafor (jane.okafor@example.com, acct 4471) reported a failed transfer.
after:   Customer «PERSON_1» («EMAIL_1», «ACCT_1») reported a failed transfer.
```

{/* DIAGRAM: redaction before/after — styled HTML panel, synthetic text */}
...
Here is the shape of a single run, with synthetic details so nothing real leaks.
...
{/* DIAGRAM: representative-workflow sequence — the run above, including the self-validation loop */}
...
{/* DIAGRAM: eval harness — fixtures (known-good) → run blind → grade against answer → score per model */}
...
Read that and the decision writes itself. Pulling and summarizing a single source is easy enough that a small local model does it for nearly free; correlation and the final verdict are where judgment lives, so those earn the bigger model. The point isn't "local good, cloud bad." It's that you can only make the trade deliberately once you've measured it, and most people never measure it.
...
None of this is specific to investigations, or to the six tools I happened to wire up. The pattern is the portable part: specialized agents over one generalist, a correlation step that earns its conclusions, a validation loop that distrusts the easy answer, a structured contract instead of prose, and a blind eval that right-sizes the models underneath it. Swap the sources and it's a different system doing the same honest work.
...
**Why not just one big, capable model with a long prompt?** Because it drifts, and because it's the most expensive way to do the easy 80% of the work. Small specialized agents are more predictable and cheaper to rerun, and they let you spend the big model only where judgment actually lives.
